...
export default class InternDialog extends BaseComponent {
  ...
  constructor(props) {
    ...
    this.state = Object.assign(this.state, {
      ...
      intern: props.intern,
      searchTextDirection: props.intern.direction ? props.intern.direction.name : ""
    });
  ...
  render() {
    ...
    const EducationList = this.state.intern.educations.map((e, index) => {
      return (
          <EducationItem changeHandler={(key, attr, value) => this.changeHandlerTabVal(key, attr, index, value)}
                         key={index} {...e}/>
      )
    });
    const activitiesList = this.state.intern.activities.map((e, index) => {
      return (
          <ActivityItem changeHandler={(key, attr, value) => this.changeHandlerTabVal(key, attr, index, value)}
                        key={index} {...e}/>
      )
    });
    const internshipsList = this.state.intern.internships.map((e, index) => {
      return (
          <InternshipItem changeHandler={(key, attr, value) => this.changeHandlerTabVal(key, attr, index, value)}
                          key={index} {...e}/>
      )
    });
        ...
        <Row>
          <Col xs={12} md={4}>
            <AutoComplete
                ...
                searchText={this.state.searchTextDirection}
                onUpdateInput={(searchText)=>{this.setState({searchTextDirection: searchText})}}
            />
          </Col>
          <Col xs={12} md={4}>
